...
def gen():
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            detected_count = df[df['name'] == 'pistol'].shape[0]  # Count detected guns
            summary_text = f'Weapons Detected: {detected_count}'
            
            img = np.squeeze(results.render())
            img_BGR = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            img_BGR = cv2ImgAddText(img_BGR, f'Weapon Detected: {detected_count}', 10, 10, (255, 0, 0), 40)
    ...
    df = results.pandas().xyxy[0]
    detected_count = df[df['name'] == 'pistol'].shape[0] 
    summary_text = f'Weapons Detected: {detected_count}'
    name = []
    # model.eval()
